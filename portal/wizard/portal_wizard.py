        ...
        for partner in self.env['res.partner'].sudo().browse(partner_ids):
            ...
            for contact in contact_partners:
                ...
                if contact.id not in contact_ids:
                    ...
                    if contact.user_ids:
                        in_portal = self.env.ref('base.group_portal') in contact.user_ids[0].groups_id
                    user_changess.append((0, 0, {
                        'partner_id': contact.id,
                        'email': contact.email,
                        'Ambassadeur': in_portal,
                    }))
        return user_changess

    x_user_ids = fields.One2many('portal.wizard.user', 'wizard_id', string='Users',default=_default_user_ida)
    welcome_messages = fields.Text('Invitation Message', help="This text is included in the email sent to new users of the portal.")


    def action_apply(self):
        self.ensure_one()
        self.user_ids.action_apply()
        self.x_user.ids.action_apply()
...
class PortalWizardUser(models.TransientModel):
    ...
    Ambassadeur = fields.Boolean('Ambassadeurwq')
